...
public class EditActivity extends AppCompatActivity {
    ...
    String kategoriTerpilih = "Motor", kendaraanTerpilih = "", keperluan = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = dbHelper.getSewayuById(getIntent().getExtras().getInt("id"));
        cursor.moveToFirst();
        kategoriTerpilih = cursor.getString(2);
        kendaraanTerpilih = cursor.getString(3);
        keperluan = cursor.getString(5);
        daritanggal.setText(cursor.getString(6));
        hinggatanggal.setText(cursor.getString(7));
        ketersediaanBensin = cursor.getInt(9);
        ...
        for (String s : keperluanArr) {
            switch (s) {
                case "- Event":
                    ...
                    break;
                case "- Jalan - jalan":
                    jalan.setChecked(true);
                    break;
                case "- Ngedate":
                    ngedate.setChecked(true);
                    break;
                case "- Acara Keluarga":
        ...
        if (kategoriTerpilih.equals("Motor")) {
        ...
        seekBar.setProgress(ketersediaanBensin);
        ...
        try {
            Date date = formatter.parse(daritanggal.getText().toString());
            tglDari.setTime(date);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        try {
            Date date = formatter.parse(hinggatanggal.getText().toString());
            tglSampai.setTime(date);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        ...
        if(validasiInput()) {
            ...
            builder.setPositiveButton("OKE", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    ...
                    boolean input;
                    input = dh.updateSewayu(sewayu, getIntent().getExtras().getInt("id"));
                    if (input) {
                        Toast.makeText(getApplicationContext(), "Berhasil memperbaharui sewa", Toast.LENGTH_SHORT).show();
                    } else {
